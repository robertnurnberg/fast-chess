...
void Match::verifyPvLines(const Player& us) {
    const static auto verifyPv = [&us](Board board, const std::string& startpos, const std::vector<std::string>& uci_moves,
        ...
        // then see convertScoreToString for converting to plies
        ...
        if (score_type == engine::ScoreType::MATE) {
            uint64_t plies = score > 0 ? score * 2 - 1 : score * -2;
            std::string warning;
            if (pv->size() < plies) {
                warning = "Warning; Incomplete mating PV";
            } else if (pv->size() > plies) {
                warning = "Warning; Too long mating PV";
            } else {
                movegen::legalmoves(moves, board);
                if (!moves.empty() || !board.inCheck()) {
                    warning = "Warning; Mating PV does not end with checkmate";
                }
            }
            ...
            Logger::print<Logger::Level::WARN>("{1}{0}{2}{0}{3}{0}{4}", separator, warning, uci_info, position, ucimoves);
        }

     };

    for (const auto& info : us.engine.getInfoLines()) {
