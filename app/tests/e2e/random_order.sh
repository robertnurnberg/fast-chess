...
./fastchess -engine cmd=./random_mover name=random_move_1 -engine cmd=./random_mover name=random_move_2 \
    ...
    -openings file=./app/tests/data/short.epd format=epd order=random -srand 1 \
    -pgnout file=random_run1.pgn -log file=log.txt level=info 2>&1 | tee "$output_file"
check_no_engine_runtime_errors "$output_file"

output_file=$(mktemp)
./fastchess -engine cmd=./random_mover name=random_move_1 -engine cmd=./random_mover name=random_move_2 \
    -each tc=2+0.02s -rounds 3 -repeat -concurrency 1 \
    -openings file=./app/tests/data/short.epd format=epd order=random -srand 2 \
    -pgnout file=random_run2.pgn -log file=log.txt level=info 2>&1 | tee "$output_file"
check_no_engine_runtime_errors "$output_file"
